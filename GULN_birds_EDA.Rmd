...
# Count # of individuals detected per species-loc-survey. Need to add in zero's!
template <- merge(
  df_sitecovs %>% dplyr::select(location_name, event_date) %>% dplyr::distinct(),
  sort(unique(df_finaldat$species_code)))

test <- df_finaldat %>%
  full_join(template, by = c("site_name", "unit_code", "location_name", ""))
...
df_summary_counts <- df_finaldat %>%
  dplyr::select(site_name, unit_code, location_name, protocol, yr, count, species_code, common_name, scientific_name, breeder, within_year_visit, event_date) %>%
    ...
    dplyr::mutate(
      ...
      yr_visit = factor(paste0(yr, "_", within_year_visit))) %>%
    dplyr::ungroup() %>%
...
## Species Summaries
Table showing average detections/survey by species and year. Row is species (sorted highest total first) and column is year. Need to account for zeroes. 

SELECT A SPECIES
```{r}  
shared_summary_counts <- SharedData$new(df_summary_counts, group = "species_code") 

filter_select(id = "species_code", label = "Select a Species", sharedData = shared_summary_counts, multiple = FALSE, group = ~species_code)

```

```{js}
# Set default values for crosstalk
function filter_default(){
  document.getElementById("species_code").getElementsByClassName("selectized")[0].selectize.setValue("NOCA",false) 
}
