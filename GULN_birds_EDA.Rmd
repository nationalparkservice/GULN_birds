...
total_page_ht <- 150 + 15 * length(min(survey_wk_heatdat$wk):max(survey_wk_heatdat$wk)) * num_facet_rows + 10 * num_facet_rows
...
```{r}
# >>>>>>>>>>>>> I NEED TO SET THE COLORS CONSISTENT ACROSS SUBPLOTS
survey_loc_heatdat <- df_survey_conditions %>% 
  ...
  dplyr::summarize(
    median_wk = median(wk)) %>%
  dplyr::full_join(merge(unique(.[c("unit_code", "location_name")]), data.frame(yr = min(.$yr):max(.$yr)))) %>%
...
# set colorbar range
vir_colors <- scales::viridis_pal(direction = -1)(max(survey_loc_heatdat$median_wk, na.rm = TRUE) - min(survey_loc_heatdat$median_wk, na.rm = TRUE) + 1)
...
subplot_loc <- . %>% plot_ly(
    ...
    colors = vir_colors,
    limits = c(min(survey_loc_heatdat$median_wk, na.rm = TRUE), max(survey_loc_heatdat$median_wk, na.rm = TRUE)),
    type = "heatmap",
    text = ~hover_text,
    hoverinfo ="text"
    ) %>%
