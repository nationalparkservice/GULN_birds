...
FuncYrBoxPlotly <- function(dat, freeY) {
  ...
  subplot_box <- . %>% 
    ...
    layout(
      ...
      yaxis = list(
        ...
        autorange = ifelse(freeY, TRUE, FALSE),
        zeroline = T
      )
    ) %>%
  add_annotations(
  ...
  dat %>%
    ...
    subplot(
      nrows = NROW(.), 
      shareX = FALSE,
      shareY = ifelse(freeY, FALSE, TRUE),
      titleX = FALSE,
      titleY = TRUE,
      margin = c(0.05, 0.05, 0.01, 0.04), # this is the subplot margin, L R T B
      which_layout = 1) %>%
    layout(
      title = paste0("Counts per survey for ", unique(dat$species_codename), " (each jittered point is a survey)"),
...
subplot_counts <- . %>% plot_ly(
  ...
  layout(
    ...
    xaxis = list(
      ...
      tickmode = "array",
      ticktext = sort(as.character(unique(dat$yr_visit))),
      tickvals = sort(as.character(unique(dat$yr_visit))),
      autorange = FALSE,
      tickfont = list(size=10)
      )) %>%
  add_annotations(
...
cat("line259")
  rv$dat <- df_full_obs_cov %>%
    dplyr::filter(unit_code %in% input$sel_park & species_codename ==  input$sel_species)
  cat("line262")
...
df_full_obs_cov <- df_full_obs %>%
  ...
  dplyr::mutate(
    ...
    hover_text = paste0("<span style='font-size:16px; font-weight:bold;'>", unit_code, " ", yr, "<br>", paste0(scientific_name, " / ", common_name, " (", species_code, ")"), "</span><br>Location: ", location_name, "<br>Habitat Type: ", hab_type, "<br>Survey Date: ", event_date, "<br># of Individuals: ", sum_indiv))
...
Summary of Covariate Relationships
=========================================
...
renderUI({
  ...
  selectInput(
    ...
    choices = sort(unique(df_full_obs_cov$species_codename)),
    selected = switch(is.null(input$sel_species)+1, input$sel_species, "Northern Cardinal (NOCA)")
    )
  })
...
renderUI({
  checkboxInput("sel_freeY",
                        ...
                        value = switch(is.null(input$sel_freeY)+1, input$sel_freeY, TRUE))
  })
```
...
* <font size="2"> Counts are the number of unique individuals detected during a point count, WITHOUT CORRECTING FOR DETECTION PROBABILITY. </font>

* <font size="2"> Red line is loess smooth. Does NOT account for detection probability or differences in survey locations across years. </font>

* <font size="2"> Blah </font>

```{r}
output$out_BoxPlot <- renderPlotly({
  shiny::req(!is.null(rv$dat), !is.null(input$sel_freeY), !is.null(input$sel_species), !is.null(input$sel_park))
  cat("line597")
  FuncYrBoxPlotly(dat = rv$dat, freeY = input$sel_freeY)
})
...
cat("line619")
  FuncHeatCountPlotly(dat = rv$dat)
})
...
```{r}
output$out_GroupedBox <- renderPlot({
  shiny::req(!is.null(rv$dat), !is.null(input$sel_species))

  grouped_box <- lapply(c("hab_type", "weather_wind", "weather_sky", "weather_noise"), function(z) {
    ggplot(data = subset(rv$dat, !is.na(get(z))),  aes(x=unit_code, y=sum_indiv, fill = get(z), color = get(z))) + 
      labs(title = switch(z, "hab_type" = "Covariate: Habitat Type", "weather_wind" = "Covariate: Wind", "weather_sky" = "Covariate: Weather Conditions", "weather_noise" = "Covariate: Ambient Noise"), x = NULL, y = "Count per Survey") +
      geom_boxplot(alpha = 0.2, position = position_dodge(preserve = "single")) + # relatively new 'preserve' argument--nice! 
      scale_fill_viridis_d(name = switch(z, "hab_type" = "habitat type", "weather_wind" = "wind", "weather_sky" = "weather conditions", "weather_noise" = "ambient noise")) +
      scale_color_viridis_d() +
      theme_bw(base_size = 16) +
      theme(legend.position = "right") +
      guides(color = "none") # remove the duplicate color legend
  }
  )
  
  temperature_points <- 
    ggplot(data = rv$dat, aes(x = weather_temperature, y = sum_indiv)) + 
    labs(title = "Covariate: Air Temperature (C)", x = "Air Temperature (C)", y = "Count per Survey", subtitle = "Point size is scaled to number of data points") +
    geom_count(alpha = 0.5) +
    theme_bw(base_size = 16) +
    facet_wrap(vars(unit_code), ncol = 3)
  
  cov_plots <- patchwork::wrap_plots(grouped_box, ncol = 2)
  
  ppage <- patchwork::wrap_plots(
      cov_plots, temperature_points, nrow = 2, heights = c(2, ceiling(length(unique(rv$dat$unit_code))/3)))
 
   ppage + plot_annotation(title = paste0("Distribution of ", input$sel_species, " Counts Across Covariate Levels"), subtitle = "Plots are NOT interactive",
    theme = theme(plot.title = element_text(size = 22), plot.subtitle = element_text(size = 18)))
}, height = 1800)

tags$style("#out_GroupedBox{height:100vh;overflow-x:scroll;overflow-y:scroll}")
plotOutput('out_GroupedBox', height="100%", width = "90%")
```

### Patterns in Detection Distance
...
renderPlotly({
  shiny::req(!is.null(df_finaldat), !is.null(input$sel_park), !is.null(input$sel_species))
  subdat <- df_finaldat %>%
    dplyr::mutate(species_codename = paste0(common_name, " (", species_code, ")")) %>%
    dplyr::filter(unit_code %in% input$sel_park, species_codename == input$sel_species) ### <<<<<<<<<<<<<<<<< PICK UP FROM HERE
  ggplot(data = rv$dat, aes(x = dis))
  )
})
ggplot(tt_raw, aes(x= distance_bin_id, weight = count)) + geom_bar()
```

### Patterns in Time-to-Detection
...
output$out_TreePlot <- renderPlot({
  ...
  partykit::ctree(sum_indiv ~ as.factor(unit_code) + as.factor(hab_type) + weather_wind_num + weather_sky_revised_num + weather_temperature + weather_noise_num + julian_prop, data = rv$dat) %>% plot(.)
  })
