...
```{r}
# TO DO:
# > Which plots are truly useful--maybe not the heat plots?
# > Survey dates (julian prop.) for earlier years vs post-review. May need to restrict use of some 2010 data.
# > Covariate plots... 
```
...
FuncDetectPlot <- function(dat, colname) {
...
```{r}
unique(df_full_obs_cov[c("unit_code", "location_name", "hab_type")]) %>% dplyr::select(unit_code, hab_type) %>% table() %>% knitr::kable()
```

Timing of surveys (Julian date) by park unit
```{r}
df_survey_conditions %>%
  ...
  layout(
    ...
    yaxis = list(
      title = "Survey Date (as proportion of full year)",
      zeroline = T
    )
  )
```
...
```{r}
FuncDetectYrPlot(dat = shared_full_obs, colname = "unit_code")
```
