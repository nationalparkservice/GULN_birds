...
df_veg <- read_csv(here::here("Data_in", "GULN_AllParks_BBird_Veg_061522.csv")) %>%
  janitor::clean_names("snake") %>%
  dplyr::mutate(hab_type = case_when(
    physiognomy %in% c("brush", "Deciduous Shrubland", "Evergreen Shrubland", "Shrublands", "grassland", "Fields and Croplands", "Forbland", "Introduced Grassland", "Marsh", "Mowed Lawn", "prairie", "Roads and Trails")  ~ "open",
    physiognomy %in% c("Deciduous Forest", "Deciduous Woodland", "Woodland and Forest", "woodlands and Forest", "Woodlands and Forest") ~ "forest")
  )
...
df_survey_conditions <- read_csv(here::here("Data_in", "GULN_AllParks_BBird_SiteConditions_2019-2021.csv")) %>%
  janitor::clean_names("snake") %>%
...
# Only keep point locations that are actually surveyed
df_locs %<>% dplyr::filter(location_name %in% unique(df_finaldat$location_name))
...
test <- subset(df_survey_conditions, unit_code == "BITH")
