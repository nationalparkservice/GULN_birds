...
# > GUIS split out into GUIS-FL and GUIS-MS in all files
...
yr_visit_template <- df_finaldat %>%
  ...
  dplyr::select(-within_year_visit) %>%
  dplyr::distinct()
...
rm(df_locs)
rm(df_full_obs) # 5473272 bytes/ survey cond=130392 bytes; veg=36224 bytes
```
